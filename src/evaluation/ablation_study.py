#!/usr/bin/env python3
"""
Ablation study for ADANN_LightGBM model.
Tests the impact of data augmentation, epochs, and other parameters.
"""
...
class AblationStudy:
    ...
    def __init__(self, output_dir="outputs/ablation_study"):
    ...
    def run_augmentation_ablation(self):
        ...
        configs = [
            ...
            {'enabled': True, 'augment_factor': 1, 'name': 'Light Augmentation'},
            {'enabled': True, 'augment_factor': 2, 'name': 'Medium Augmentation'},
            {'enabled': True, 'augment_factor': 3, 'name': 'Heavy Augmentation'}
        ]
        ...
        for config in configs:
            ...
            # Run training
            try:
                result = self._run_training_experiment(config['name'])
    ...
    def run_epochs_ablation(self):
        """Test the impact of training epochs."""
        print("🔬 Running epochs ablation study...")
        
        epochs_list = [25, 50, 100, 150, 200]
        
        for epochs in epochs_list:
            print(f"  Testing: {epochs} epochs")
            
            try:
                result = self._run_training_experiment(f"{epochs}_epochs", epochs=epochs)
    ...
    def run_n_trials_ablation(self):
        """Test the impact of Optuna trials."""
        print("🔬 Running n_trials ablation study...")
        
        n_trials_list = [25, 50, 100, 200]
        
        for n_trials in n_trials_list:
            print(f"  Testing: {n_trials} trials")
            
            try:
                result = self._run_training_experiment(f"{n_trials}_trials", n_trials=n_trials)
                self.results['n_trials_ablation'].append({
                    'n_trials': n_trials,
                    'result': result
                })
            except Exception as e:
    ...
    def _create_temp_config(self, config):
        ...
        if not config['enabled']:
            full_config['data']['augmentation']['enabled'] = False
        else:
            full_config['data']['augmentation']['enabled'] = True
            full_config['data']['augmentation']['augment_factor'] = config['augment_factor']
    ...
    def _run_training_experiment(self, name, epochs=100, n_trials=50):
        ...
        cmd = [
            ...
            '--output_suffix', f'ablation_{name}'
        ]
    ...
    def create_ablation_plots(self):
        ...
        # N_trials ablation plot
        if self.results['n_trials_ablation']:
            self._plot_n_trials_results()
    ...
    def _plot_augmentation_results(self):
        ...
        if accuracies:
            x = range(len(configs))
            ax1.bar(x, accuracies, color='skyblue', alpha=0.7)
            ax1.set_title('Accuracy by Augmentation Level')
            ax1.set_ylabel('Accuracy (%)')
            ax1.set_xticks(x)
            ax1.set_xticklabels(configs, rotation=45)
            
            ax2.bar(x, f1_scores, color='lightcoral', alpha=0.7)
            ax2.set_title('F1-Score by Augmentation Level')
    ...
    def _plot_n_trials_results(self):
        """Plot n_trials ablation results."""
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
        ...
        for result in self.results['n_trials_ablation']:
            if 'error' not in result['result']:
        ...
        if accuracies:
            ax1.plot(n_trials, accuracies, 'o-', color='green', linewidth=2, markersize=8)
            ax1.set_title('Accuracy vs Optuna Trials')
            ax1.set_xlabel('Number of Trials')
            ax1.set_ylabel('Accuracy (%)')
            ax1.grid(True, alpha=0.3)
            
            ax2.plot(n_trials, f1_scores, 'o-', color='orange', linewidth=2, markersize=8)
            ax2.set_title('F1-Score vs Optuna Trials')
            ax2.set_xlabel('Number of Trials')
            ax2.set_ylabel('F1-Score (%)')
            ax2.grid(True, alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(f'{self.output_dir}/n_trials_ablation.png', dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def save_results(self):
        ...
        results_path = f"{self.output_dir}/ablation_results_{timestamp}.json"
        ...
        for result in self.results['n_trials_ablation']:
            if 'error' not in result['result']:
                summary_data.append({
                    'Test': 'N_Trials',
                    'Config': f"{result['n_trials']} trials",
                    'Accuracy': result['result']['accuracy'],
                    'F1_Score': result['result']['f1_score']
                })
        ...
        if summary_data:
            ...
            summary_path = f"{self.output_dir}/ablation_summary_{timestamp}.csv"
...
def main():
    ...
    print("="*60)
    print("ADANN_LightGBM ABLATION STUDY")
    print("="*60)
    ...
    ablation.run_n_trials_ablation()
