"""
BSL Gesture Recognition - 1D-DSCNN Model Definition
"""
...
class Cnn1dModelCreator:
    ...
    def create_model(self, params, arduino_mode=False, callbacks=None):
        ...
        if arduino_mode:
            model.add(layers.SeparableConv1D(16, 3, activation='relu'))
            model.add(layers.MaxPooling1D(2))
            model.add(layers.SeparableConv1D(32, 3, activation='relu'))
            model.add(layers.MaxPooling1D(2))
            ...
            for i in range(params['n_conv_layers']):
                model.add(layers.SeparableConv1D(params[f'conv{i+1}_filters'], params[f'conv{i+1}_kernel'], activation=params['activation']))
