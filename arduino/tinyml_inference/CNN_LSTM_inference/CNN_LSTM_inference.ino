...
const float CONFIDENCE_THRESHOLD = 0.75;  // CNN_LSTM更高阈值
const int STABLE_PREDICTION_COUNT = 4;    // CNN_LSTM需要更多稳定性

// TensorFlow Lite配置
constexpr int tensorArenaSize = 40 * 1024;  // CNN_LSTM需要约40KB
...
void checkForGestureStart() {
  ...
  if (totalVariance > GESTURE_TRIGGER_THRESHOLD) {
    ...
    gestureFrameCount = 0;
    Serial.println("🎯 CNN_LSTM检测到可能的手势开始");
    digitalWrite(LED_PIN, HIGH);
  }
}
...
void confirmGestureStart() {
  ...
  if (gestureFrameCount >= MIN_GESTURE_DURATION) {
    ...
    Serial.println("✅ 手势确认，开始CNN_LSTM完整推理");
...
void checkGestureEnd() {
  ...
  if (totalVariance < STATIC_THRESHOLD) {
    ...
    stateStartTime = millis();
    Serial.println("📉 CNN_LSTM手势结束，进入冷却期");
  }
}

void checkCooldownEnd() {
  if (millis() - stateStartTime > 1500) {  // CNN_LSTM冷却期稍长
    currentState = STATE_STATIC;
...
void outputGestureResult(int gestureClass, float confidence) {
  ...
  if (currentTime - lastOutputTime < 600) return;  // CNN_LSTM输出频率稍低
  
  Serial.print("🤲 CNN_LSTM识别结果: ");
...
void performCompleteInference() {
  ...
  // CNN_LSTM需要更多计算时间，频率稍低
  if (currentTime - lastInferenceTime < 300) return;  // 约3.3Hz
...
void collectSensorData() {
  ...
  if (!bufferFilled && bufferIndex == 0) {
    bufferFilled = true;
    Serial.println("📊 CNN_LSTM数据缓冲区已填充完成");
  }
}
...
void printHelp() {
  Serial.println("=== CNN_LSTM手势识别系统命令帮助 ===");
  ...
  Serial.println();
  Serial.println("CNN_LSTM特点：");
  Serial.println("- 更高的复杂手势识别精度");
  Serial.println("- 时序特征和空间特征结合");
  Serial.println("- 需要更多计算时间和内存");
}
...
void printBufferInfo() {
  Serial.println("=== CNN_LSTM数据缓冲区信息 ===");
  Serial.print("状态: ");
...
void setup() {
  ...
  Serial.println("=== BSL手势识别系统 - CNN_LSTM智能推理版本 ===");
  ...
  Serial.println("正在初始化CNN_LSTM模型...");
  if (!modelInit(model, tensorArena, tensorArenaSize)) {
    Serial.println("❌ CNN_LSTM模型初始化失败!");
    while (true) {
      digitalWrite(LED_PIN, HIGH);
      delay(200);
      digitalWrite(LED_PIN, LOW);
  ...
  Serial.println("✅ CNN_LSTM模型初始化成功");
  ...
  Serial.println("智能推理模式：静态监测 -> 手势检测 -> CNN_LSTM推理");
