...
// Precomputed features for inference-only timing
...
static void cmd_help() {
  ...
  Serial.println("  latency R W         - run latency with R runs and W warmup");
...
static void run_latency(int runs, int warmup) {
  ...
  // Classifier instance from micromlgen port
  ...
  double latency_us = s_pred.avg_v;               // microseconds
  double throughput = 1000000.0 / s_pred.avg_v;   // inferences per second
...
void setup() {
  ...
  Serial.println("XGBoost Inference-Only Latency Test Ready. Type 'help'.");
}
...
void loop() {
  if (Serial.available()) {
    ...
    if (line.equalsIgnoreCase("help")) {
      ...
      precompute_features();
      Serial.println("OK: recorded fixed window & precomputed features");
