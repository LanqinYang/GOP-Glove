...
static void quantize_pack_from_window(const float src[SEQUENCE_LENGTH][NUM_SENSORS]) {
  int total = input->bytes; // for int8 this equals element count
  int idx = 0;
  ...
  while (idx < total) input->data.int8[idx++] = input->params.zero_point; // pad
}

static void run_latency(int measure_runs, int warmup_runs) {
  if (!has_fixed_window) { Serial.println("ERR: No fixed window. Run 'record' first."); return; }
  ...
  for (int i = 0; i < warmup_runs; ++i) {
    ...
    quantize_pack_from_window(ema_buf);
    interpreter->Invoke();
  }
  ...
  // compute-only latency & throughput
  struct Stats s_infer; compute_stats(t_infer, measure_runs, &s_infer);
  double latency_ms = s_infer.avg_v / 1000.0;      // us -> ms
  double throughput = 1000000.0 / s_infer.avg_v;   // inferences per second
...
void setup() {
  ...
  setup_tf();
  Serial.println("Transformer Encoder latency tester ready. Type 'help'.");
}
...
void loop() {
  if (Serial.available()) {
    ...
    if (line.equalsIgnoreCase("help")) {
      cmd_help();
    } else if (line.equalsIgnoreCase("record")) {
      capture_window(window_fixed);
      has_fixed_window = true;
      Serial.println("OK: recorded fixed window");
    } else if (line.startsWith("latency")) {
      int runs = RUNS_DEFAULT, warm = WARMUP_DEFAULT;
      int sp = line.indexOf(' ');
      if (sp > 0) {
        int sp2 = line.indexOf(' ', sp + 1);
        if (sp2 > 0) {
          runs = line.substring(sp + 1, sp2).toInt();
          warm = line.substring(sp2 + 1).toInt();
        } else {
          runs = line.substring(sp + 1).toInt();
        }
      }
